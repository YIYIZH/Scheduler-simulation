...
int  SJF(task tasks[], int nbOfTasks, int currentTime) {
 ...
 int duration = 0;
 int shortest = 0;
  // Otherwise, find the shortest job
  for(i=0; i<nbOfTasks; i++) {
    if (tasks[i].state == RUNNABLE) {
      duration =  tasks[i].totalOfComputations;
      break;
    }
  }   
  for(i=0; i<nbOfTasks; i++) {
    if (tasks[i].state == RUNNABLE) {     
      if (tasks[i].totalOfComputations <= duration){
        duration =  tasks[i].totalOfComputations;
        shortest = i;
      }
    }
  }
  if (tasks[shortest].state == RUNNABLE && tasks[shortest].computationsDone < tasks[shortest].totalOfComputations) {
    tasks[shortest].computationsDone ++;
    tasks[shortest].state = RUNNING;
    return shortest;
  }
...
/* Returns the index of the selected task */
/* Return -1 if no task could be selected */
int  PSJF(task tasks[], int nbOfTasks, int currentTime) {
  int i;
 ...
 int shortest = 0;
  ...
  for(i=0; i<nbOfTasks; i++) {
	...
	tasks[i].state = TERMINATED;
	break;
      } else if (tasks[shortest].totalOfComputations < tasks[i].totalOfComputations - tasks[i].computationsDone && tasks[shortest].state == RUNNABLE){
                tasks[i].state = RUNNABLE;
		//tasks[i].totalOfComputations = tasks[i].totalOfComputations - tasks[i].computationsDone;
                tasks[shortest].state = RUNNING;
                tasks[shortest].computationsDone ++;
		return shortest;
...
int main(int argc, char *argv[]){
  ...
  while(hasTasksToSchedule(tasks, nbOfTasks) > 0) {
    ...
    taskIndex = PSJF(tasks, nbOfTasks, time);
    if (taskIndex >= 0) {
